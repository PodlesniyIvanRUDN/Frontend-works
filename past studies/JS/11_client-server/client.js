...
//sort by Name
function sortByName( studentsList, direction, handlers){
    ...
    renderStudentsTable(studentsList, handlers);
    return direction;
}

//sort by birthday
function sortByBirthday( studentsList, direction, handlers){
    for(let i = 0; i < studentsList.length; i++){
        for(let j = 0; j < studentsList.length-i-1; j++){
...
//sort by date
function sortByDate( studentsList, direction, handlers){
    for(let i = 0; i < studentsList.length; i++){
        for(let j = 0; j < studentsList.length-i-1; j++){
...
function sortByFaculty( studentsList, direction, handlers){
    console.log('faculty sort direction', direction);
    for(let i = 0; i < studentsList.length; i++){
        for(let j = 0; j < studentsList.length-i-1; j++){
            console.log(studentsList[j].faculty, ' === ', studentsList[j+1].faculty)
...
function selectSort(id,  studentsList, direction,handlers){

    switch(id){

        case 'fio-sort':
            direction = sortByName(studentsList,direction, handlers);
            return direction;

        case 'bd-sort':
            direction =  sortByBirthday(studentsList,direction, handlers);
            return direction;

        case 'ed-sort':
            direction =  sortByDate(studentsList,direction, handlers);
            return direction;

        case 'faculty-sort':
            direction = sortByFaculty(studentsList,direction, handlers);
            return direction;
...
function filterAll(desiredStudent, filters, studentsList){
    ...
    if (filters === 0){
        ...
        if(filtrations === filters){
            console.log('CLEAR 1', removeElements);
        ...
        if(desiredStudent.finish !== ''  ){
            for(let student of filtered){
                console.log('YEP ', typeof student.studyStart)
                if(Number(student.studyStart) + 4 !== Number(desiredStudent.finish)){
                    removalIndex = student;
                    if(removeElements.includes(removalIndex) === false)
                        removeElements.push(removalIndex);
                    }
        ...
        if(filtrations === filters){
            console.log('CLEAR 2', removeElements);
            if(removeElements.length === filtered.length)
            return '';
            else{
                filtered = filtered.filter(item => !removeElements.includes(item))
        ...
        if(desiredStudent.studyStart !== 0  ){
            ...
            for(let student of filtered){
                console.log('YEP ', typeof student.studyStart)
                if(Number(student.studyStart) !== Number(desiredStudent.studyStart)){
                    removalIndex = student;
                    if(removeElements.includes(removalIndex) === false)
                        removeElements.push(removalIndex);
                    }
        ...
        if(filtrations === filters){
            console.log('CLEAR 3', removeElements);
            if(removeElements.length === filtered.length)
            return '';
            else{
                filtered = filtered.filter(item => !removeElements.includes(item))
        ...
        if(filtrations === filters){
            console.log('CLEAR 4', removeElements);
            if(removeElements.length === filtered.length)
            return '';
            else{
                filtered = filtered.filter(item => !removeElements.includes(item))
...
let filtered = false;
let direction = true; 
const stString = await fetch(`http://localhost:3000/api/students`)
 studentsList = await stString.json();

[...studentsList].forEach(student => {
    container.append(getStudentItem(student, handlers));
});
...
sideBarForm.addEventListener('submit', function(e){
    ...
    if(needed.length !== 0 && filtered === false){
        document.querySelectorAll('.student__data').forEach(e => e.remove());
        studentsList = [...needed];
        renderStudentsTable(needed, handlers);
        filtered = true;
    }
...
unfilterButton.addEventListener('submit', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        console.log(typeof bdayYear);
            ...
            if((( Number(edStart) - bdayYear )<= 15 ) || (edStart < bdayYear) || ( Number(edStart) < 2000) ){
                clearAlert(removableObj);
                alertSpan.textContent = 'Education start date is invalid or student below 16 years old';
                alertSpan.style.color = 'red';
                formContainer.append(alertSpan);
                return;
            }
        ...
        let student = getStudentItem(studentItem, handlers);
    ...
    for(let span of spanListeners){
        ...
        span.addEventListener('click', function(){
            let id = span.id;
            console.log('span selector',id);
            direction = selectSort(id,studentsList,direction, handlers);
        })
...
function getStudentItem(studentObj, {onDelete}) {
    ...
    deleteBtnWrapper.classList.add('delete__wrapper')
    ...
    studentInfo.append(deleteBtnWrapper);
    deleteBtn.addEventListener('click', async function() {
        onDelete({studentObj, element:studentInfo})
    })
    return studentInfo;
...
function renderStudentsTable(studentsList, handlers) {
